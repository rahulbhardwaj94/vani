...
```sh
./scripts/setup-signing.sh   # one-time: creates the "Vani Dev" signing identity
./scripts/build-app.sh       # release build → build/Vani.app (signed)
open build/Vani.app
```
...
"Vani Dev" signing identity and persist across rebuilds only when signed.
...
- Ollama URL/timeout are overridable via VANI_OLLAMA_URL / VANI_OLLAMA_TIMEOUT
