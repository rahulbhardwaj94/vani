...
[![Website](https://img.shields.io/badge/Website-the--vani--app.vercel.app-D9772D)](https://the-vani-app.vercel.app)
...
- 👨‍💻 **Code mode** — in terminals and editors (VS Code, Cursor, Xcode, JetBrains, iTerm…): no auto-caps, no trailing period, spoken casing ("camel case get user name" → `getUserName`) and spoken symbols ("git commit dash m" → `git commit -m`, "ls pipe grep foo" → `ls | grep foo`, "server dot js" → `server.js`)
